function this_MD = bn_load_MD_run_filament_segmentation(this_MD, Parameter_MD, varargin)
...
% Modified 4/20/21 Benjamin Nanes
% Add noGraphics field to the 'input_parameter_set structure. 
% If noGraphics==1, suppress graphics in geoBasedNmsSeg_withGM and graph_matching_linking_once.
% Modiefied functions (as this one) are prefixed with bn_
...
if(~isempty(Parameter_MD))
                    ...
                    if(strcmp(Parameter_MD.processes_{iPro}.getName, 'Steerable filtering'))
                        ...
                        if(strcmp(Parameter_MD.processes_{iPro}.getName, 'Filament Segmentation'))
                            ...
                            if(~ismember('whole_movie_filename',ip.UsingDefaults))
                                this_MD = bn_filament_segmentation(this_MD,given_Params,whole_movie_filename);
                            else
                                this_MD = bn_filament_segmentation(this_MD,given_Params);
                            end
                            %save MD to disk
                            this_MD.save();
                        end
    ...
    if(~ismember('whole_movie_filename',ip.UsingDefaults))
        ...
        if(run_with_new_param ==1)
            this_MD = bn_filament_segmentation(this_MD,default_Params,whole_movie_filename);
        else
            this_MD = bn_filament_segmentation_continue(this_MD,default_Params,whole_movie_filename);
        end
    else        
        if(run_with_new_param ==1)
            this_MD = bn_filament_segmentation(this_MD,default_Params);
        else
            this_MD = bn_filament_segmentation_continue(this_MD,default_Params);
        end
    end    
