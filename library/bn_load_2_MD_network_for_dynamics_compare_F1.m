function [similarity_scoremap_cell,similarity_scoremap_1to2_cell,similarity_scoremap_2to1_cell, distance_map_1_2_cell, distance_map_2_1_cell, angle_map_1_2_cell, angle_map_2_1_cell] ...
    = bn_load_2_MD_network_for_dynamics_compare_F1(MD1_filename,iChannel1, start_frame1,...
    MD2_filename, iChannel2, start_frame2, ...
    radius,show_save_everything_flag,...
    longest_radius,sigma_gaussian, sigma_d, sigma_theta,...
    save_tif_flag, save_combined_flag, outdir)
% function to compare two networks
% Modified from the original, applications/cytoskeleton/filamentNetworkAnalysis/load_2_MD_network_for_dynamics_compare.m
% to make comparisons only from start_frame1 of MD1_filename (rather than all comparisons from start_frame1 to the end of the movie).
...
if(~exist('save_combined_flag','var'))
    save_combined_flag = 0;
end
...
if(~exist('outdir','var'))
    ...
    outdir = [MD.outputDirectory_,filesep,outdir];
end
...
for iFrame = start_frame1 %: nFrame % <----------- Here is the change //BN210209
    ...
    if iFrame_2<= nFrame
        ...
        try
            ...
            MT_img =  MD_1.channels_(iChannel1).loadImage(iFrame);
        end
        ...
        try
            ...
            VIF_img =  MD.channels_(iChannel2).loadImage(iFrame);
        end
...
%         display('Start plotting');
        ...
        if(save_tif_flag)
            ...
            imwrite(similarity_scoremap_8bit, [outdir,filesep,'Similarity_maps_frame',num2str(iFrame),'.tif']);
        end
